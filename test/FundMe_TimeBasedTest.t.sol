...
contract FundMe_TimeBasedTest is Test {
    ...
    function testRetrieveIfContractIsClosed() public {
    ...
    function testRetreiveWhenStatusIsComplete() public {
        address owner = s_fundMe.getOwner();
        ...
        s_fundMe.setStatus(FundMe.Status.Finished);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                timeStamp,
                timeLimit,
                uint256(FundMe.Status.Finished)
            )
        );
        s_fundMe.performUpkeep("");
    }
